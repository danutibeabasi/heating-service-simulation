...
from service_two_750watts.model import RoomModel
from service_two_750watts.controller import HeatingController

app = Flask(__name__)
controller = HeatingController()

def create_routes(app, controller):
    @app.route('/service_two/control', methods=['POST'])
    def control_heating_two():
    ...
    @app.route('/service_two/status', methods=['GET'])
    def get_status_two():
